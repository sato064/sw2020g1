...
public class SearchInfo extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 学生の検索
        stores = manager.findStores(str_name);
        // requestオブジェクトにオブジェクトを登録
        request.setAttribute("StoreList", stores);
        if (stores != null) {
            session.setAttribute("FindStoreName", str_name);
        }
        // forwardはrequestオブジェクトを引数として、次のページに渡すことができる
        System.out.println(stores);
        
        RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/FindedStores.jsp");
